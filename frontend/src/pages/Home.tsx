    ...
    <>
      ...
      <Section tone="tinted">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          <div>
            <SectionHeading
              title="What the chapter is for"
              lede={site.aims}
            />
            <div className="mt-8 space-y-6">
              <div>
                <h3 className="text-[1.05rem] text-plum-700 dark:text-primary">Our vision</h3>
                <p className="mt-2 text-[0.95rem] leading-relaxed">{site.vision}</p>
              </div>
              <div>
      ...
      <Section tone="tinted">
        <SectionHeading
          id="pricing"
          title="Event pricing"
          lede="Sign in before you check out and the member rate applies automatically."
          className="mb-8"
        />
          ...
          {standardTiers.map((tier) => (
            <div key={tier.id} className="flex flex-col bg-card p-6">
              ...
              <Button variant="outline" asChild className="mt-6 w-full">
                <Link to="/events">Choose an event</Link>
              </Button>
            </div>
          ))}
        </div>
